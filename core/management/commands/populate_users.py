...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        ...
        parser.add_argument(
            ...
            type=int,
            default=1,
            help="Quantidade de usuários a serem criados por Role (Padrão: 1).",
        )
    ...
    def handle(self, *args, **options):
        if os.getenv("NODE_ENV") != "dev":
            self.stdout.write(
                self.style.WARNING(
                    "Ignorado: populate_users só roda com NODE_ENV=dev. Nada foi criado."
                )
            )
            return
        ...
        quantidade_por_role = options["qtd"]

        for role in selected_roles:
            for _ in range(quantidade_por_role):
                prefix = role.lower()
                ...
                try:
                    ...
                    self.stdout.write(
                        self.style.SUCCESS(
                            f"Usuário {role} ({email}) criado com sucesso usando o modelo {model.__name__}!"
                        )
                    )
